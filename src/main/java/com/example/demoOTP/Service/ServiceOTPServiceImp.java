...
import com.example.demoOTP.Config.Reponse.responsecheckOTP;
...
public class ServiceOTPServiceImp implements ServiceOTPService{
    ...
    public Object randomCode(String secret, otprequest request) throws Exception {
        ...
        if(reqid != null && !reqid.equals("")){
            List<ServiceOTP> listreqid = serviceOTPRepo.findAll();
            for(int i=0 ; i<listreqid.size() ; i++){
                if(listreqid.get(i).getReqId().equals(reqid)){
                    reponseError.setError("Error Request ID");
                    reponseError.setMessage("Request ID Repeated ");
                    return reponseError ;
                }
            }
            if(!secret.isEmpty()){
    ...
    public Object CheckTimeOTP(String code, String reference, String uuid) {
        ...
        if(servicedata == null){
            reponseError.setError("Error OTP Code");
            reponseError.setMessage("OTP code not found");
            return reponseError;
        }
        ...
        if(ref.equals(reference)) {
            if(uid.equals(uuid)){
                if (TimeNow < ExpirationTime) {
                    ...
                    responsecheckOTP.setMessage("Use Code Successfully");
